...
#     }
    ...
    def update_vector_store(self, chunks: List):
        """
        Add documents to Chroma in small batches.

        Railway instances can have limited RAM. Sending hundreds of chunks
        to the embedding model and Chroma in one call can cause very slow
        processing or an out-of-memory crash.
        """
        if not chunks:
            return
        ...
        logger.info(
            f"Adding {total} new chunks to vector store "
        ...
        for start in range(0, total, batch_size):
            ...
            self.all_documents.extend(batch)

            logger.info(
                f"Completed batch {batch_number}/{total_batches}."
            )

        # Rebuild BM25 only after all Chroma batches are successfully added.
        ...
        logger.info(
            f"Vector store updated successfully with {total} new chunks."
        )
